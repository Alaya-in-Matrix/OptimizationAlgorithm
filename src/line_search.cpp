                             ...
                             double max_walk) const noexcept
{
    ...
    while(true)
    {
        ...
        double new_step_hi = cubic_predict(step_lo, sol_lo.fom(), g_lo, step_hi, sol_hi.fom(), g_hi);
        if (std::isnan(new_step_hi) || new_step_hi < 2 * step_hi) 
            new_step_hi = 2 * step_hi;
        else if(new_step_hi > max_step)
        {
            if(step_hi > (min_step + max_step) / 2)
            {
                cerr << "set step_hi as best step, curvature condition maight be violated" << endl;
                best_sol = line_func({step_hi});
                break;
            }
            else
                                  ...
                                  double g2) const noexcept
{
    ...
    VectorXd abcd = cubic_interpolation(x1, y1, g1, x2, y2, g2);
    double a      = 3 * abcd(0);
    double b      = 2 * abcd(1);
    double c      = 1 * abcd(2);
